...
//        guard let emojiSorts = NSArray(contentsOfFile: arrayPath?) as? [String] else {
//
//        }
        guard let arrayPath =  Bundle.main.path(forResource: "emoji_sort.plist", ofType: nil) else {
            print("emoji_sort.plist is not found!")
            return
        }
        
        guard let emojiSorts = NSArray(contentsOfFile: arrayPath) else {
            print("emojiSorts is null")
            return
        }
        ...
        for (index, id) in emojiSorts.enumerated() {
            ...
            temps.append(Emoji(id: id as! String, png: "emoji_\(id)_32x32_"))
        }
